...
/*
What to do next:
TODO 1. Refactor the code: deck objects, real player objects, benchmark objects,
and configuration objects, etc.
TODO 2. A more powerful deck initialization method: initialize the deck with a
configuration object.
TODO 3. Draw line charts in a more reasonable way.
*/
...
class wssim {
   ...
   public:
    ...
    static bool pattackp(player &atk, player &def, int soul, int &triggered);
    static void push_cards_into_deck(std::vector<card> &deck, card card,
                                     int num);
    static int multhread_sim(int (*func)(player, player, int),
                             const player &atk, const player &def,
                             const int count = 10000, int thread_num = 1,
                             int inner_repeat = 1);
    static void basic_bench(int (*sim)(player &, player &),
...
class player {
   public:
    ...
    void init_defender(int _hp, int _deck, int _climax_in_deck,
...
std::vector<card> wssim::shuffle_copy(std::vector<card> deck) {
    for (int i = 0; i < deck.size(); i++) {
        std::uniform_int_distribution<> dist_deck(i, deck.size() - 1);
...
bool wssim::pattackp(player &atk, player &def, int soul, int &triggered) {
    triggered = atk.deck.back().trigger;
    int damage = soul + atk.deck.back().trigger;
    atk.stock.push_back(atk.deck.back());
    atk.deck.pop_back();
    atk.refresh_check();
...
void player::init_defender(int _hp, int _deck, int _climax_in_deck,
    ...
    if (_deck > 50) {
        std::cout << "Warning: deck is larger than 50, it's " << _deck
                  << " now." << std::endl;
    }
    ...
    wssim::push_cards_into_deck(deck, card(0, card::CLIMAX, 0),
                                _climax_in_deck);
...
void player::print() {
    ...
    for_each(waiting_room.begin(), waiting_room.end(),
             [](auto x) { x.print(); });
